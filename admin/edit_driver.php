...
$id = $_GET['id'];
$query1 = mysqli_query($con, "SELECT * FROM driver WHERE id = '$id'");
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['imagename']) && $_FILES['imagename']['error'] == UPLOAD_ERR_OK) {
        ...
        $aimageTemp = $_FILES['imagename']['tmp_name'];

        // Sanitize the new image file name
        $aimage = preg_replace("/[^a-zA-Z0-9\.\-\_]/", "", $aimage);
        $targetDir = "drivers/";
        $targetFile = $targetDir . basename($aimage);
        ...
        if (!move_uploaded_file($aimageTemp, $targetFile)) {
            echo "<script>alert('Error uploading the new image.');</script>";
            exit();
        }
    } else {
        $aimage = $row['image']; // If no new image is uploaded, keep the old image
    }

    $sql = "UPDATE driver SET fname = '$fname', phone = '$phone', email = '$email', car_name = '$car_name', 
            t_sit = '$t_sit', car_num = '$car_num', ass_locat = '$ass_locat', image = '$aimage' WHERE id ='$id'";

    $query = mysqli_query($con, $sql);

    if ($query) {
        echo "<script>alert('Driver Updated Successfully ✔️✔️✔️'); window.location='driver_list.php';</script>";
    } else {
                                    ...
                                    <div class='row'>
                                        <div class='col-lg-6'>
                                            <div class='common_input mb_15'>
                                                <input type='text' placeholder='FullName' name='fname' value="<?php echo $row['fname']; ?>" required>
                                            </div>
                                        </div>
                                        <div class='col-lg-6'>
                                            <div class='common_input mb_15'>
                                                <input type='number' placeholder='Mobile No' name='phone' value="<?php echo $row['phone']; ?>" required>
                                            </div>
                                        </div>
                                        <div class='col-lg-6'>
                                            <div class='common_input mb_15'>
                                        ...
                                        <div class='col-lg-6'>
                                            <div class='common_input mb_15'>
                                                <input type='text' placeholder='Name of Car' name='car_name' value="<?php echo $row['car_name']; ?>" required>
                                            </div>
                                        </div>
                                        <div class='col-lg-6'>
                                            <div class='common_input mb_15'>
                                                <input type='number' placeholder='Total Sit' name='t_sit' value="<?php echo $row['t_sit']; ?>" required>
                                            </div>
                                        </div>
                                        <div class='col-lg-6'>
                                            <div class='common_input mb_15'>
                                                <input type='text' placeholder='Car Number' name='car_num' value="<?php echo $row['car_num']; ?>" required>
                                            </div>
                                        </div>
                                        <div class='col-lg-6'>
                                            <div class='common_input mb_15'>
                                                ...
                                                <img src="drivers/<?php echo $row['image']; ?>" style='width:200px; height:100px;'>
                                            </div>
                                        </div>
                                        <div class='col-lg-6'>
